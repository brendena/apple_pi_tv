...
* (Done) raspberry pi uart route to debug header
* (Done) change pico's uart to a different uart output
* (Done) LED way to bright
* (Done) removed the HDMI enable.  Were never going to not have it
* (Done) the hole near hdmi is has different spacing between apple tv V2,V3
* (Future) fix footprint to toss link, 
  * see if the part is different between v2 and v3 
* (Done)LED holder for front LED wrong
* (Try-Done) Fix power input to pi
  * currently i need to set it to 4.7 V to get it to work
  * see if it's the PG pin
* (Try-Done) Move IR sensor.
  * make sure it works with both V2, V3
* (Done) HID i2c needs a irq line
* (Done) Add i2c lines for HID device
  * Add pull up resisters
* (Future) SD card semi boots
  * moved D2 under
